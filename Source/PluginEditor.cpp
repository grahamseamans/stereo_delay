    ...
    : AudioProcessorEditor(&p), audioProcessor(p)
{
    ...
    float stepFilter = 1;
    initSlider(feedbackSlider, feedbackLabel, "Feedback", 0, 1, 0.01, audioProcessor.feedback);
    initSlider(rightDelaySlider, rightDelayLabel, "Right Delay", minDelay, maxDelay, stepDelay, audioProcessor.rightDelay);
    initSlider(leftDelaySlider, leftDelayLabel, "Left Delay", minDelay, maxDelay, stepDelay, audioProcessor.leftDelay);
    initSlider(rightFilterSlider, rightFilterLabel, "Right Filter Cutoff", minFilter, maxFilter, stepFilter, audioProcessor.rightFilter);
    initSlider(leftFilterSlider, leftFilterLabel, "Left Filter Cuttoff", minFilter, maxFilter, stepFilter, audioProcessor.leftFilter);

    rightFilterSlider.setSkewFactorFromMidPoint(400);
    leftFilterSlider.setSkewFactorFromMidPoint(400);
}

void Delay_PluginAudioProcessorEditor::initSlider(juce::Slider &slider, juce::Label &label, juce::String text, float min, float max, float step, float setValue)
{
